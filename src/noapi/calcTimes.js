...
export default function calcTimes(method) {
	//------------------------ Constants --------------------------
	...
	*/
	...
	for (let i in methods) {
		...
		for (let j in defParams) if (typeof params[j] == "undefined") params[j] = defParams[j]
	}
	...
	return {
		...
		sunAngleTime: function (angle, time, direction) {
			...
			let t = (1 / 15) * DMath.arccos((-DMath.sin(angle) - DMath.sin(decl) * DMath.sin(lat)) / (DMath.cos(decl) * DMath.cos(lat)))
