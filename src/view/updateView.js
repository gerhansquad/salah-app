...
export default function updateView(state) {
	...
	;(function updateTime() {
		...
		if (currentTime > currentDayPrayerDataArray[4]) {
			// time has passed isha prayertime -> prev prayer is prevday's isha
			...
			for (let index = 0; index < currentDayPrayerDataArray.length - 2; index++) {
				if (currentTime < currentDayPrayerDataArray[index]) {
					if (index === 0) {
						//if its the first prayer -> prev prayer is prevday's isha
